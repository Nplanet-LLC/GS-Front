...
import { columns, type NewsBooking } from "./columns";
...
const RegistrationSubscriptionPage = () => {
  const [data, setData] = useState<NewsBooking[]>([]);
  ...
  const fetchData = async (page: number = currentPage) => {
    setLoading(true);
    try {
      const response = await axios.get(`${url}News/get-all-bookingNews`);
      ...
      console.error("Error fetching newsletter bookings:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = data.filter(
    (item) =>
      ...
      item.newsLetterName.toLowerCase().includes(filterText.toLowerCase())
  );
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-6 rounded-lg shadow-sm">
          <div>
            ...
            <p className="text-sm text-gray-600">
              View and manage all newsletter registrations
            </p>
          </div>
          <div className="w-full md:w-1/3 relative">
            <input
              type="text"
              placeholder="Search by email, name or newsletter..."
