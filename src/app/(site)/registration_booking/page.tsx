...
export default function NewsletterPage() {
  ...
  const onSubmit = (data: any) => {
    const selectedCountries = countryFields.map(
      (index) => data[`country_${index}`]
    );

    console.log("data:", data);
    console.log("Selected Countries:", selectedCountries);
    console.log("Total:", selectedCountries.length * 250);
    setPrice(selectedCountries.length * 250);
  ...
  useEffect(() => {
    const selectedCount = watchedCountries.filter((c) => c?.value).length;
      ...
      >
        ...
        <div>
          ...
          <button
            ...
            className="text-blue-600 underline mt-3 font-semibold cursor-pointer"
          >
            Add Country
          </button>
        </div>
